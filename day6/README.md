...
Now we are using the `Exclude` type to filter out the naughty children. `Exclude` takes two type parameters, the first is the union of all the children's naughty/nice status, and the second is the thing we want to exclude from the final resulting type. In other words, it creates a type that includes all types that are in `T` but not in `U`.
