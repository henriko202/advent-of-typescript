...
Now we have a generic type using the `<T>` syntax, `keyof T` part is an indexed type query, which results in a type that represents all possible keys of type `T`.
